...
| `dam_stats_stddev_2016_2025.csv` | SPI standard deviation within detected water surface (high value = permanent core + seasonal fringe) |
...
3. Review and adjust thresholds in **Section 9** if needed (see below)
4. Run all cells
...
| 3. Load, merge, spatial join | Read CSVs, merge on `dam_id` + `year`, compute water area, assign admin1/admin2/locality |
| 4. Inspect | Regional mean water area per year — use to confirm reference year |
| 5. Pivot to wide format | One row per dam, one column per metric per year |
| 6. Reference year normalisation | Compute `relative_water_fraction` against per-dam reference year |
| 7. Trend analysis | Mann-Kendall test + Sen's slope for water fraction and SPI series |
| 8. Territorial summary | Water availability statistics by admin1 and admin2 |
| 9. Inspect distributions | Inspect distributions and adjust `WF_HIGH`, `WF_LOW`, `SPI_PERM`, `SPI_SEAS` |
| 10. Non-functional dams | Extract dams with `wf_mean < WF_LOW` across the full record |
| 11. Classification | Assign Set 1 and Set 2 classes + restoration candidate flag |
| 12. Plots | Pekel-style time series plots by class |
| 13. Export | Save classified inventory to `outputs/` |
